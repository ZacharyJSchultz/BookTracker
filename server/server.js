...
const fetchQuery = "SELECT * FROM Books ORDER BY dateCompleted DESC";
...
app.post("/submit-form", (req, res) => {
    try {
        ...
        let book_id = -1;
        con.execute(checkBookExistsQuery, [formData.title, formData.author], (err, rslt) => {
            if (!err && rslt !== null)
                book_id = rslt;
        });
...
app.get("/view-db", (req, res) => {
    try {
        con.query(fetchQuery, function(err, rslt) {     // TODO: UPDATE. SET ALL GENRES TO FALSE EXCEPT ONES WITH ID
            if (err)
                throw err;
